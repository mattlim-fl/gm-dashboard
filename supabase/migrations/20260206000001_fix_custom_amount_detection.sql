-- Fix Custom Amount detection - these items have null name but item_type = 'CUSTOM_AMOUNT'
-- This adds ~2 more items to attendance count
  ...
  WITH src AS (
    SELECT DISTINCT ON (o.order_id)
      ...
      (
        ...
        WHERE
          -- Named door items (RE-ENTRY explicitly NOT included)
          (li->>'name') = ANY(ARRAY[
            'Hippie Entry',
            'Complimentary Entry',
            'Free Entry',
          ...
          -- OR custom amount items (name is null but item_type indicates custom)
          OR (li->>'item_type') = 'CUSTOM_AMOUNT'
      ) AS door_ticket_qty,
...
COMMENT ON FUNCTION public.transform_orders_window IS 'Transforms raw Square orders into orders table. door_ticket_qty counts: Hippie Entry, Complimentary Entry, Free Entry, Megatix, and CUSTOM_AMOUNT items. Excludes RE-ENTRY.';
