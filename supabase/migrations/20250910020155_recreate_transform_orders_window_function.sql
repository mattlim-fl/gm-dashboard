-- Drop and recreate the transform_orders_window function
DROP FUNCTION IF EXISTS public.transform_orders_window(timestamptz, timestamptz);

-- Create the transform_orders_window function that sync-and-transform expects
CREATE OR REPLACE FUNCTION public.transform_orders_window(p_start timestamptz, p_end timestamptz)
RETURNS integer
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  -- Process orders that were created within the time window
  WITH src AS (
    SELECT
      o.order_id,
    ...
    WHERE (o.raw_response->>'created_at')::timestamptz >= p_start
      AND (o.raw_response->>'created_at')::timestamptz <= p_end
  )
