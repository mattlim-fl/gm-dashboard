-- Ensure orders table exists with necessary columns
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='orders' AND column_name='door_ticket_qty'
  ) THEN
    RAISE EXCEPTION 'orders table missing or schema differs';
  END IF;
END $$;

-- Improve transform function to upsert into public.orders and compute door_ticket_qty from line_items
...
BEGIN
  FOR r IN
    SELECT
      o.order_id,
      o.raw_response,
      o.raw_response->'order'->>'location_id' AS location_id,
  ...
  LOOP
    ...
    VALUES (
      ...
      r.location_id,
      venue_name,
      r.raw_response->'order'->>'state',
      (r.raw_response->'order'->>'created_at')::timestamptz,
      (r.raw_response->'order'->>'updated_at')::timestamptz,
      NULLIF(r.raw_response->'order'->'total_money'->>'amount','')::int,
      (
        SELECT COALESCE(SUM(
                 CASE
        ...
        FROM jsonb_array_elements(COALESCE(r.raw_response->'order'->'line_items','[]'::jsonb)) li
        WHERE (li->>'name') = ANY(door_names)
      ),
      r.raw_response->'order'->'line_items',
      now()
    )
